...
server <- function(input, output, session) {
  ...
  #### Prospective report 6 mo ####
  
  data_prospective6 <- reactive({
    data_prospective6 <- contact_info() %>%
      filter(indicator == "A") %>%
      filter(
        !is.na(update_expected) |
          !is.na(corrections_expected) |
      ...
      mutate(update_flag = ifelse(
        Date %within%
          interval(Sys.Date(), Sys.Date() %m+% months(6)),
        1,
        0
      )) %>%
      filter(update_flag == 1) %>%
    ...
    data_prospective6 <- rename(data_prospective6, NIH = Check)
    
  })
  
  output$prospective6 <- downloadHandler(
    filename = function() {
      paste("ctgov_prospective_data_6_", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
